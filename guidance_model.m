...
function sys=mdlDerivatives(t,x,u,P)
  ...
  % solve for psi
  psi =  chi - asin(1/Va*(we*cos(chi)-wn*sin(chi)));
  ...
  pndot   =  Va*cos(psi) + wn;
  pedot   =  Va*sin(psi) + we;
  chiddot =  b_chidot*(-chidot)+b_chi*(chi_c-chi);
  Vadot   =  b_va*(Va_c-Va);

  % don't let climb rate exceed Va*sin(\gamma_max)
  gamma_max = 30*pi/180;
  hddot   = b_h*(-hdot)+b_h*(h_c-h);
